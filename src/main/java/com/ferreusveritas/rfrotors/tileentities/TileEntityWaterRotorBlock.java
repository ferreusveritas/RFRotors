...
public class TileEntityWaterRotorBlock extends TileEntityRotorBlock {
	...
	private FluidFlow getFlowPatternPlane() {
		...
		for(int dv = 2; dv >= -2; --dv) {
			...
			for(int du = -2; du <= 2; ++du) {
				...
				int dx = du * Math.abs(rotorDir.getFrontOffsetZ());
				int dy = dv;
				int dz = du * Math.abs(rotorDir.getFrontOffsetX());
				
				BlockPos dPos = pos.add(dx, dy, dz);
				Block block = world.getBlockState(dPos).getBlock();
				
				if( !((dx | dy | dz) == 0) && !block.isReplaceable(world, dPos)){
				...
				if(f != null) {
					IBlockState liquidState = world.getBlockState(dPos);
					Block liquidBlock = liquidState.getBlock();
					
					if(liquidBlock instanceof BlockLiquid) {
						int meta = liquidState.getValue(BlockLiquid.LEVEL);
						// meta >= 8 means falling block, which we don't care about
						if(meta < 8) {
							int fDepth = 8 - (meta & 7);
		...
		if(rotorDir.getFrontOffsetZ() != 0) {
			// xy-plane so u is x direction, i.e. East/West
			if(gradient > 0) {
				flow.direction = EnumFacing.EAST;
			} else if(gradient < 0) {
				flow.direction = EnumFacing.WEST;
			} else {
				flow.direction = null;
			}
		} else {
			...
			if(gradient > 0) {
				flow.direction = EnumFacing.SOUTH;
			} else if(gradient < 0) {
				flow.direction = EnumFacing.NORTH;
			} else {
				flow.direction = null;
			}
		}
	...
	protected void burn(){
		...
		for(BlockPos iPos : BlockPos.getAllInBox(pos.up().offset(rotorDir.rotateY()), pos.down().offset(rotorDir.rotateY().getOpposite()))) {
			world.setBlockState(iPos, fireState);
		}
				
		EntityItem burningWheel = new EntityItem(world, pos.getX() + 0.5f, pos.getY() + 0.5f, pos.getZ() + 0.5f, new ItemStack(ModBlocks.rotorBlock, 1, BlockRotor.EnumType.WATERROTORWOOD.getMetadata()));
		burningWheel.setFire(5);
		world.spawnEntity(burningWheel);
	}
	...
	public EnergyPacket getEnergyPacket() {
		...
		if(getType() == BlockRotor.EnumType.WATERROTORWOOD && flow.type.getTemperature() > 450) {//Ignition point of wood in celsius
			burn();
			return new EnergyPacket(0, 0);
		}
		...
		for(int i = 0; i < sampleFlows.length; ++i) {
			...
			if(Integer.bitCount(tmp) < 3) {
				...
				if(i == 0){//Undershot
					// Undershot
					float boost = getType() == BlockRotor.EnumType.WATERROTORWOOD ? 1.5f : 1f;//Wooden water wheels can be a bit more efficient due to their low mass 
					speed *= ((flow.direction == EnumFacing.NORTH || flow.direction == EnumFacing.EAST) ? -1 : 1) * boost;
					energy = calcUndershotEnergy(flow) * boost;
				} else {
					speed *= ((rotorDir == EnumFacing.NORTH || rotorDir == EnumFacing.EAST) ? -1 : 1);
				...
				speed = MathHelper.clamp(speed, -5.0f, 5.0f);
